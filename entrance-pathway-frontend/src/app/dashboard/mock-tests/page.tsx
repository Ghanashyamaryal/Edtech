      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {testsToDisplay.slice(0, 10).map((exam) => {
              const latestAttempt = latestAttemptMap.get(exam.id);
              const hasAttempted = !!latestAttempt;
              const latestScore = latestAttempt?.score || 0;
              const latestPercentage = exam.totalMarks > 0 ? Math.round((latestScore / exam.totalMarks) * 100) : 0;
              const passed = latestScore >= exam.passingMarks;
              ...
              <div className="flex gap-2">
                      <Link href={`/dashboard/mock-tests/${exam.id}`} className="flex-1">
                        <Button className="w-full gap-2" variant={hasAttempted ? "outline" : "default"}>
          ...
          <Card>
            <CardContent className="py-8 text-center">
              ...
              <p className="text-muted-foreground">
                You&apos;ve attempted all available tests! You can still retake them to improve your score.
              </p>
            </CardContent>
          </Card>
        ) : (
        ...
        {testsToDisplay.length > 10 && (
          <div className="text-center mt-4">
            <Link href="/dashboard/mock-tests/all">
              <Button variant="outline" className="gap-2">
                View All Tests ({testsToDisplay.length})
                <ArrowRight className="w-4 h-4" />
              </Button>
            </Link>
          </div>
